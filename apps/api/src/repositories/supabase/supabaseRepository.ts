...
export class SupabaseRepository implements DataRepository {
  ...
  async getProfile(userId: string): Promise<Profile> {
    const { data } = await supabase
      .from("profiles")
      .select(
        "id, full_name, email, profile_completion, education_level, nationality",
      )
      .eq("id", userId)
  ...
  async listSavedOpportunities(userId: string): Promise<Opportunity[]> {
    ...
    return ((data ?? []) as SupabaseSavedOpportunityRow[])
      .map((row) => row.opportunities?.[0] ?? null)
      .filter(Boolean)
      .map((opportunity) =>
        toOpportunity(opportunity as SupabaseOpportunityRow),
      );
  }
  ...
  async getDashboard(userId: string): Promise<DashboardPayload> {
    const [
      ...
      supabase
        .from("activity_feed")
        .select("id, title, status, date_label")
        .eq("user_id", userId)
    ...
    const applications = (applicationsResult.data ??
      []) as SupabaseApplicationRow[];
    const activity = ((activityResult.data ?? []) as SupabaseActivityRow[]).map(
      (row) => ({
        id: String(row.id),
        title: row.title,
        status: row.status,
    ...
    const fallbackRecommended = (
      (recommendationsResult.data ?? []) as SupabaseOpportunityRow[]
    ).map(toOpportunity);

    return {
      stats: {
        inProgress: applications.filter((item) => item.status === "in-progress")
          .length,
        submitted: applications.filter((item) => item.status === "submitted")
          .length,
        awarded: applications.filter((item) => item.status === "awarded")
          .length,
